...
/* the LORIKEET reacts to light.
 * the lighter it gets, the more chirps it emits in a row
 * the lorikeet also gets tired from chirping, so also takes a proporionnally
 * long rest after long chirps
 * when it gets darker, the chirps are less repeated, but iterated over
 * faster
 */
